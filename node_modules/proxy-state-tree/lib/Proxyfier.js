...
const getValue = (proxyOrValue) => proxyOrValue && proxyOrValue[exports.IS_PROXY] ? proxyOrValue[exports.VALUE] : proxyOrValue;
...
class Proxifier {
    ...
    ensureValueDosntExistInStateTreeElsewhere(value) {
        if (process.env.NODE_ENV === 'production')
            return;
        if (value && value[exports.IS_PROXY] === true) {
            throw new Error(`proxy-state-tree - You are trying to insert a value that already exists in the state tree on path "${value[exports.PATH]}"`);
        }
        return value;
    }
    trackPath(path) {
    ...
    isProxyCached(value, path) {
        return (value[this.CACHED_PROXY] &&
            String(value[this.CACHED_PROXY][exports.PATH]) === String(path));
    }
    createArrayProxy(value, path) {
        ...
        const proxy = new Proxy(value, {
            get(target, prop) {
                if (prop === exports.IS_PROXY)
                    return true;
                if (prop === exports.PATH)
                    return path;
                if (prop === exports.VALUE)
                    return value;
                if (prop === 'indexOf') {
    ...
    proxify(value, path) {
        if (value) {
            const isUnmatchingProxy = value[exports.IS_PROXY] &&
                (String(value[exports.PATH]) !== String(path) ||
                    value[exports.VALUE][this.CACHED_PROXY] !== value);
            if (isUnmatchingProxy) {
                return this.proxify(value[exports.VALUE], path);
            }
            else if (value[exports.IS_PROXY]) {
                return value;
            }
